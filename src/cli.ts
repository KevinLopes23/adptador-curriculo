...
function loadResumeBase(): Resume {
  ...
  if (!existsSync(resumePath)) {
    console.error(
      `[ERRO] Arquivo resume_base.json nao encontrado em: ${resumePath}`,
    );
    console.error(
      '       Execute "npm run extract" para gerar o JSON do curriculo.',
    );
    process.exit(1);
  }
...
function readJobDescriptionFromFile(filePath: string): string {
  ...
  if (!existsSync(fullPath)) {
    console.error(
      `[ERRO] Arquivo de job description nao encontrado: ${fullPath}`,
    );
    process.exit(1);
  }
...
program
  ...
  .option(
    "-o, --output <path>",
    "Caminho do PDF de saida",
    "./output/curriculo.pdf",
  )
  .option(
    "--ats",
    "Gerar PDF otimizado para sistemas ATS (sem cores, alto contraste)",
    false,
  )
  .action(async (options: { job?: string; output: string; ats: boolean }) => {
    try {
      ...
      console.log(
        `      Experiencias: ${resumeBase.experienciaProfissional.length}`,
      );
      ...
      if (jobDescription.length < 50) {
        console.error(
          "[ERRO] Job description muito curta. Forneça uma descricao mais detalhada.",
        );
        process.exit(1);
      }
      ...
      const adaptedResume = await geminiService.adaptResume(
        resumeBase,
        jobDescription,
      );
